  ...
  return <main className="strength-page">
    ...
    {activeTab === "plan" && <section className="strength-plan-overview strength-tab-panel" id="weekly-strength" role="tabpanel" aria-labelledby="weekly-strength-title">
      ...
      <div className="strength-plan-overview__grid">
        ...
        <details className="is-optional"><summary><span>Sunday</span><strong>Upper / Aesthetic</strong><small>25–35 min · Optional</small></summary><label className="strength-sunday-choice">Abdominal exercise<select value={store.selectedAbExercise} onChange={(event) => onSelectAb(event.target.value as TrainingStore["selectedAbExercise"])}><option value="cable-crunch">Cable crunch</option><option value="hanging-knee-raise">Hanging knee raise</option><option value="reverse-crunch">Reverse crunch</option></select></label><ol>{[...strengthSessions[2].exercises, selectedAb].map((exercise) => <li key={exercise.id}><ExerciseReference exercise={exercise.name} label="View" row prescription={`${exercise.sets} × ${exercise.minReps}–${exercise.maxReps}${exercise.repLabel ? ` ${exercise.repLabel}` : ""}`} /></li>)}</ol><p>Choose only one abdominal exercise. Current selection: <strong>{selectedAb.name}</strong>.</p></details>
      </div>
    </section>}
