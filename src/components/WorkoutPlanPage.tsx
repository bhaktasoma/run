...
export default function WorkoutPlanPage({ store, onSaveLog, onSaveDecision, onSelectAb }: WorkoutPlanPageProps) {
  ...
  return <main className="strength-page">
    ...
    <nav className="strength-page-nav" aria-label="Strength page sections"><a href="#weekly-strength">Weekly plan</a><a href="#strength-adjustment">This week</a>{sessions.length > 0 && <a href="#strength-logger">Log workouts</a>}<a href="#strength-guides">Guides</a></nav>

    <section className="strength-plan-overview" id="weekly-strength" aria-labelledby="weekly-strength-title">
      <header><div><p className="goal-page__eyebrow">Standard weekly schedule</p><h2 id="weekly-strength-title">Detailed strength plan</h2></div><div className="strength-plan-overview__tools"><p>{suggestedMode === "post-race" ? "Reference only during post-race recovery. Resume progressively when normal walking and daily movement are comfortable." : "Use the recovery adjustment below when it differs from this standard plan."}</p><label>Sunday abdominal choice<select value={store.selectedAbExercise} onChange={(event) => onSelectAb(event.target.value as TrainingStore["selectedAbExercise"])}><option value="cable-crunch">Cable crunch</option><option value="hanging-knee-raise">Hanging knee raise</option><option value="reverse-crunch">Reverse crunch</option></select></label></div></header>
    ...
    {sessions.length ? <>
      <section className="strength-week-heading" id="strength-logger"><div><p className="goal-page__eyebrow">This week</p><h2>Workout loggers</h2></div><p>Open the session you are completing. Start with 2–3 good repetitions in reserve; do not routinely train to failure.</p></section>
    ...
    <section className="strength-education-grid" id="strength-guides">
